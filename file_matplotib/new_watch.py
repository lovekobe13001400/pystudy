...
price, money, x, sname, sid, pzdf_max, pzdf_min, money_max, money_min, money_all_half_slope, price_all_half_slope = stock_today(1)
...
def update(val):
    ...
    func_stock_info(stock)
    func_stock_info(stock)
    pass
def func_stock_info(sn):
    price, money, x, sname, sid, pzdf_max, pzdf_min, money_max, money_min, money_all_half_slope, price_all_half_slope = stock_today(sn)
    title_str = "%s(%s)最高涨幅%s，最低涨幅%s,最大入单%s，最大出单%s" % (sname, sid, pzdf_max, pzdf_min, money_max, money_min)
    ax1.set_title(title_str, fontsize=13)
    #ax2.set_title("第二天k线,最高涨幅%s 最低涨幅%s" % (next_pzdf_max, next_pzdf_min))
...
slider_stock = Slider(data_stock, '股票', 0, 80, valinit=1, valstep=1)
