...
url =  'http://47.75.51.71:12345/api/proxy/?count=30&scheme=HTTP&anonymity=anonymous'
...
while(1):
    ...
    all_ip = json_content['data']['detail']

    for one_ip in all_ip:
        ip = one_ip['ip']
        port = one_ip['port']
        scheme = one_ip['scheme']
        handler_obj = {}
        value = ip + ':' + port
        ...
        try:
            ...
            if ip_exists is None:
                sql = 'insert ignore into ip values(null,%(ip)s,%(port)s,%(scheme)s,%(success_num)s,%(fail_num)s,%(last_time)s)'
                params = {'ip': ip, 'port': port, 'scheme': scheme, 'success_num': 1, 'fail_num': 0,'last_time':today_timestamp}
                #ip是否存在，存在更新，失败录入
